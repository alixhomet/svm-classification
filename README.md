...
À partir du second dataset, les étapes récurrentes (validation croisée, évaluation, grid search) sont **factorisées dans des fonctions réutilisables** (`do\_cv`, `do\_cm\_cr`, `do\_grid\_search`).
...
# 3. Placer les fichiers CSV (breast\_cancer, spambase, car, mushroom)
#    à la racine du projet, puis lancer Jupyter
jupyter notebook SVM\_classification.ipynb
```
...
```
.
├── SVM\_classification.ipynb   # Notebook principal
├── requirements.txt
├── README.md
└── data/                      # Fichiers CSV
